...
- [75%:dedf9a6c] Establish an architecture where embedders are strictly responsible for providing any required host interfaces, including filesystems, network egress policies, and clock resolution controls; README, support, and threat docs now all say this directly.
...
- [94%:1baa1d65] Document platform limitations, performance tradeoffs, and security assumptions in `SUPPORT_MATRIX.md` and `THREAT_MODEL.md`, including secure-mode fallback behavior, observer overhead, the current compiler fuel trap-mapping caveat, the current yielded-call fuel-observer semantics around resume, the current trap-observer resume semantics, and host-call policy observer ordering / handoff on resumed-segment denied calls.
