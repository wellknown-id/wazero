...
| 3. Deterministic CPU metering ("fuel") | [88%:63795383] | Fuel controllers, host fuel APIs, exhaustion handling, and compiler function-entry plus loop-header metering are wired through the production compiler path; interpreter debits native function entry and backward branches with nested-call, yield/resume, and host-interleaved validation. Config-level fuel clamping/round-trip, module-engine fuel initialization (local vs imported), fuel observer lifecycle (Consumed/Exhausted/absent), and full E2E runtime fuel exhaustion (interpreter + compiler/secure-mode) are now verified. Compile-time overhead measured at ~8.5%, execution overhead at ~0-1%, accounting determinism verified (fac(20) = 100 fuel units). Basic-block injection and broader cost-model stability still remain. |
...
| 5. Zero-trust host interface | [99%:0614a2bf] | Core crates keep WASI/system behavior outside the runtime. Import ACLs, fail-closed resolution, observer/audit events, `HostCallPolicy`/`YieldPolicy` surfaces with full caller-module metadata, signature-based enforcement, import-resolver observer lifecycle events, `FuelObserver` lifecycle notifications, `TimeProvider` surface, and `YieldObserver` wiring are in place, and the README/support/contributing docs now make the embedder-owned host-interface model explicit. `razero-wasm` and `razero` no longer depend on `razero-platform` in production builds (`GuardPageError` and `compiler_supported`/`supports_guard_pages` are re-exported transitively through `razero-secmem` and `razero-compiler`), `razero-secmem` is optional behind a `secure-memory` feature gate in `razero-wasm` (interpreter builds have zero `libc`), `razero-compiler` has no direct `libc` dependency (all syscalls in `razero-platform`), and the `filecache` module is feature-gated so no filesystem I/O compiles by default. `THREAT_MODEL.md` now includes an explicit crate coupling taxonomy. Broader observer composition and remaining lifecycle coverage still remain, but resumed-segment observer/provider handoff semantics are now much better pinned by public tests and summarized in one explicit support-matrix contract, including resume-time `HostCallPolicy` and `YieldPolicy` override handoff and omission semantics, direct-call ordering plus resume-time `YieldPolicyObserver` handoff, direct-host and imported-host policy denial short-circuiting later yield observers, and the current non-reinjected `Snapshotter` behavior on resumed host calls. |
| 6. Validation, hardening, and operational readiness | [99%:5db9ff2c] | Parity/spec/smoke tests, listener surfaces, benches, packaging ABI docs, policy/fault-path coverage, compiler-backed secure-mode OOB validation, trap observer API, interpreter and compiler trap-observer coverage, full public surface parity, import-resolver config/ACL round-trips, public runtime tests for import-resolver observer ordering plus yield-observer resume semantics, and broad AMD64 codegen coverage. Broader hardening depth and fallback breadth still remain; the fuzz workspace now includes policy-denial negative-path coverage for cooperative yield and host-call denial, relevant observer event-stream parity, deterministic replay coverage, a fixed-fixture trap target spanning multiple trap families, interpreter-mode fuel observer parity coverage, and fixed import-resolver observer ordering coverage, and targeted runtime tests now pin the current fuel-observer yielded/resume behavior, current fuel-controller resume behavior, fuel-observer ordering around listener `before` / `after` plus the current host-overdraw `Consumed`-then-`Exhausted` sequence, including the direct public host-call overdraw ordering path, direct host-call controller-override precedence and post-callback fuel exhaustion on public host calls, interpreter-mode guest-import host callbacks seeing the runtime fuel budget, interpreter-mode guest loop fuel exhaustion surfacing `TrapCause::FuelExhausted` through `TrapObserver`, trap-observer resume behavior, `Resumer::cancel()` remaining outside trap-observer callbacks in both parity and the main runtime suite, plain host panics staying outside trap-observer callbacks in both parity and the main runtime suite, resume-path `TimeProvider` handoff and omission, resume-path `FuelController` handoff and omission, resume-path `YieldObserver` handoff and omission, including repeated `Resumed` delivery on repeated successful resumes and no extra callback on cancel, resume-path `HostCallPolicy` and `YieldPolicy` override handoff and omission, current resumed-segment `FuelObserver` silence, including yielded-call observations finishing before any later resume, current non-reinjected `Snapshotter` behavior on resumed host calls, explicit fresh-resumer behavior across re-yield plus spent-resumer reuse rejection, suspended-call rejection while an earlier resumer is outstanding, retryable wrong-result-count resume rejection, idempotent cancel blocking later resume, and trap-observer ordering ahead of listener abort on denied guest imports in the main runtime suite, main-runtime pre-resume validation semantics for `YieldObserver` (no `Resumed` event, resumer still retryable), host-call listener ordering on denied imports, guest-import host-call-policy observer ordering ahead of trap observation, guest-import policy-observer event ordering ahead of trap observation in parity, main-runtime `HostCallPolicy` omission coverage plus `HostCallPolicyObserver` handoff and omission coverage on resumed segments, main-runtime `YieldPolicyObserver` allow/deny handoff plus omission coverage on resumed segments, main-runtime `TrapObserver` handoff and omission coverage on resumed segments, both allowed and denied guest-import paths now pinning the absence of separate imported-host listener callbacks in the main runtime suite, direct-host and imported-host host-call denial short-circuiting later yield observers, and both direct-call and resume-path host-call policy observer behavior plus direct-call ordering and resume-path handoff for `YieldPolicyObserver`, including the main runtime test for allowed-yield ordering ahead of `YieldObserver`, so support docs do not overclaim resume-time callbacks or policy-hook ordering. |
| 7. AOT packaging and native distribution | [99%:a6e60bab] | Strong Linux ELF/AOT packaging path with extensive negative coverage for malformed metadata, truncation, and corruption across all sidecar fields. The linked runtime support surface now also has explicit public fail-closed coverage for constructor, `call_export`, and `start` errors around missing exports, wrong arity, non-function exports, malformed local-function/type metadata, and invalid first-function pointers. `link_native_executable` now accepts a generalized `host_imports` parameter and `validate_linked_runtime_metadata` permits function imports, enabling arbitrary host-import packaging beyond the `hello-host` special case. Broader runtime-state packaging hardening still remains. |
...
- [80%:63795383] Validate that compiler overhead, execution overhead, and accounting accuracy are acceptable for the experiment. Module-engine fuel initialization correctness validated (local gets fuel, imported does not, disabled skips stale values). Compile-time overhead measured at ~8.5% (secbench/fuel_compile_overhead), execution overhead within noise (~0-1%), fuel exhaustion trap path ~10%. Accounting accuracy verified: fac(20) consumes exactly 100 fuel units deterministically. Basic-block injection and broader cost-model stability validation still remain.
...
- [85%:4888107a] Establish invariants for what host functions may do while a module is suspended, including rejected reentry, stale/spent resumers, host-result arity validation across re-yield, cross-module isolation during suspension, and yield-resume cycle stability.
...
- [78%:a6e60bab] Remove all WASI implementation code, OS dependencies, and system-level structures from the core engine, with the repo front door now explicitly documenting that there is no built-in runtime-owned WASI layer in core crates, and with `razero-wasm` and `razero` no longer depending on `razero-platform` in production builds.
- [82%:a6e60bab] Establish an architecture where embedders are strictly responsible for providing any required host interfaces, including filesystems, network egress policies, and clock resolution controls; README, support, and threat docs now all say this directly, public runtime tests now pin resume-path `TimeProvider` handoff to the embedder-supplied call context, and platform/platform-adjacent symbols are re-exported through non-platform crates so production builds have no direct `razero-platform` coupling.
- [78%:a6e60bab] Explicitly fail-closed by removing platform-specific adapter layers and system wrappers that bypass host application policies, with import ACL / resolver policy now documented in the README as the expected instantiation-time default-deny surface, `razero-wasm` and `razero` production builds no longer listing `razero-platform` as a direct dependency, `razero-secmem` optional and gated behind a `secure-memory` feature in `razero-wasm`, and all `libc` syscalls centralized in `razero-platform` (no direct `libc` dependency in `razero-compiler`).
- [72%:a6e60bab] Keep contributor guidance aligned with the zero-trust design so new core changes do not silently reintroduce runtime-owned system surfaces or convenience wrappers that bypass embedder policy, with CONTRIBUTING.md now containing explicit platform-coupling guardrails.

**Exit criteria**

- [80%:0614a2bf] The core engine contains absolutely zero system or OS coupling: `razero-interp` and `razero-wasm` (without `secure-memory`) have no `libc` or `razero-platform` in their dependency trees; `razero-compiler` has no direct `libc` dependency (all syscalls centralized in `razero-platform`); `filecache` is feature-gated; OS coupling exists only in `razero-platform` itself and in crates that legitimately need it (`razero-compiler` for JIT, `razero-secmem` for guard pages).
...
- [99%:16b42271] Add targeted tests for memory fault recovery, fuel exhaustion, async resumption, and policy enforcement, with public runtime coverage now also checking import-resolver observer ordering for ACL allow/deny, resolver success, store fallback, fail-closed denial, the current yield-observer resume / cancellation semantics, the current yielded-call fuel-observer behavior around resume, current fuel-controller resume behavior, fuel-observer ordering around listener `before` / `after` plus the current host-overdraw `Consumed`-then-`Exhausted` sequence, trap-observer resume behavior, `Resumer::cancel()` staying outside trap-observer callbacks in both parity and the main runtime suite, plain host panics staying outside trap-observer callbacks in both parity and the main runtime suite, resume-path `TimeProvider` handoff and omission, resume-path `FuelController` handoff and omission, resume-path `YieldObserver` handoff and omission, current resumed-call `Snapshotter` behavior, resume-path `HostCallPolicy` and `YieldPolicy` override handoff and omission, current resumed-segment `FuelObserver` silence, the main runtime pre-resume validation contract where failed resume validation does not emit `YieldEvent::Resumed` and does not spend the resumer, denied-host-import listener ordering, guest-import host-call-policy observer ordering ahead of trap observation, the matching parity-side guest-import observer-before-trap ordering, main-runtime omission coverage for resumed-segment `HostCallPolicy` plus handoff/omission coverage for resumed-segment `HostCallPolicyObserver`, main-runtime handoff plus omission coverage for resumed-segment `YieldPolicyObserver`, main-runtime handoff plus omission coverage for resumed-segment `TrapObserver`, trap-observer ordering ahead of listener abort on denied imports, both allowed and denied guest-import paths pinning the current absence of separate imported-host listener callbacks in the main runtime suite, direct-host and imported-host host-call denial short-circuiting `YieldPolicyObserver` / `YieldObserver` before any host yield can fire, and host-call plus yield-policy observer metadata / ordering / handoff for both direct-call and resumed-segment policy decisions, including the main runtime test plus parity coverage for `YieldPolicyObserver` ordering ahead of `YieldObserver::Yielded` on allowed cooperative yields.
...
- [100%:04af62a8] `razero_compiler::runtime_support::LinkedModule` provides a
  ...
  initializers, plus data-segment / element-segment bounds overflow and
  global-initializer count mismatch.
...
- [100%:ea77dc43] The current native-link entrypoint now has targeted pre-link
  ...
  linked-runtime metadata that falls outside the packaged slice, including
  ...
  initializer types and invalid/truncated/non-integer data-offset forms, plus
  malformed element-initializer metadata (unsupported opcode, missing local
  function, empty initializer, invalid `ref.null`, trailing `ref.func` bytes,
  malformed `ref.func` index) and wrapper-generation rejection for missing
  local start-function metadata.
- [100%:ea77dc43] `razero_compiler::linker::link_hello_host_executable(...)` provides a
  **specialized** native-link flow for the existing `hello-host` example,
  ...
  data metadata rejected during sidecar decode, malformed or non-integer data
  offsets, explicit `global.get` / `ref.func` offset rejection without panic,
  plus wrong import-count / missing-memory shapes that currently fail earlier
...
- [55%:04af62a8] There is not yet a fully stable, general-purpose packaging story for:
  - [90%:a6e60bab] arbitrary host imports
...
1. **Generalize host-import packaging**
   - [95%:a6e60bab] Replace the `hello-host` special case with a reusable host-ABI/runtime
      support layer, with `link_native_executable` now accepting `host_imports` and generating dispatch loops.
   - [95%:a6e60bab] Make imported functions resolve through explicit packaged host descriptors
      instead of generated ad hoc example logic, with `PackagedHostImportDescriptor` now wired through the linker.
   - [95%:a6e60bab] Keep host ownership explicit: the packager should wire host APIs supplied by
...
- [80%:0614a2bf] A minimalist, pure WebAssembly core engine completely decoupled from system, OS, and WASI dependencies: interpreter-only builds have zero `libc` in the dependency tree.
