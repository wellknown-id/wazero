...
- [72%:a6e60bab] Explicitly fail-closed by removing platform-specific adapter layers and system wrappers that bypass host application policies, with import ACL / resolver policy now documented in the README as the expected instantiation-time default-deny surface, and with `razero-wasm` and `razero` production builds no longer listing `razero-platform` as a direct dependency (`GuardPageError` re-exported from `razero-secmem`, `compiler_supported`/`supports_guard_pages` from `razero-compiler`, `filecache` behind a feature gate).
...
- [40%:a6e60bab] The core engine contains absolutely zero system or OS coupling.
...
- [60%:a6e60bab] A minimalist, pure WebAssembly core engine completely decoupled from system, OS, and WASI dependencies.
