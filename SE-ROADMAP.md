...
- [96%:1baa1d65] Document platform limitations, performance tradeoffs, and security assumptions in `SUPPORT_MATRIX.md` and `THREAT_MODEL.md`, including secure-mode fallback behavior, observer overhead, the current compiler fuel trap-mapping caveat, the current yielded-call fuel-observer semantics around resume, the current trap-observer resume semantics, resume-path `TimeProvider` handoff semantics, current resumed-call `Snapshotter` limitations, and host-call policy observer ordering / handoff on resumed-segment denied calls.
