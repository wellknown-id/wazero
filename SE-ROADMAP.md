...
| 2. Hardware-assisted memory sandboxing              | [100%:dedf9a6c] Virtual memory reservation and `mmap` guard-page prototype are in place; portability/fallback work remains.                          | [85%:db4603a6] Guarded allocation and Linux-first reserved-memory paths exist in Rust, secure-mode compiler codegen now honors memory isolation, guarded-memory SIGSEGV handling reports `MEMORY_FAULT`, compiler-backed trap observation now distinguishes secure-mode memory faults from software OOB traps on supported targets, and the live store path now falls back to plain memory on unsupported guard-page platforms; wider platform hardening and native arm64 validation still lag. |
