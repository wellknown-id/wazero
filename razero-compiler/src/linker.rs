...
"#,
    ...
    #[test]
    fn packaged_module_sidecar_rejects_corrupted_element_segment_mode() {
        let module = Module {
            type_section: vec![function_type(&[], &[ValueType::I32])],
            function_section: vec![0],
            table_section: vec![Table {
        ...
        assert_eq!(metadata.element_segments[0].init_expressions.len(), 1);

        let mut sidecar = serialize_aot_metadata(&metadata);
        let encoded_init_expr_len = 4 + metadata.element_segments[0].init_expressions[0].len();
        let mode_offset = sidecar.len() - 1 - 4 - encoded_init_expr_len;
        sidecar[mode_offset] = 0xff;
        ...
        let err = crate::aot::deserialize_aot_metadata(&decoded.modules[0].metadata_sidecar_bytes)
            .unwrap_err();
        assert_eq!(err.to_string(), "aot metadata: invalid element mode");
    }

    #[test]
    fn packaged_module_sidecar_rejects_corrupted_element_segment_ref_type() {
        let module = Module {
            type_section: vec![function_type(&[], &[ValueType::I32])],
            function_section: vec![0],
            table_section: vec![Table {
        ...
        let mut sidecar = serialize_aot_metadata(&metadata);
        let encoded_init_expr_len = 4 + metadata.element_segments[0].init_expressions[0].len();
        let ref_type_offset = sidecar.len() - 2 - 4 - encoded_init_expr_len;
        sidecar[ref_type_offset] = 0xff;
        ...
        let err = crate::aot::deserialize_aot_metadata(&decoded.modules[0].metadata_sidecar_bytes)
            .unwrap_err();
        assert_eq!(err.to_string(), "aot metadata: invalid element ref type");
    }
    ...
    #[test]
    fn packaged_module_sidecar_rejects_corrupted_active_element_ref_type_mismatch() {
        let module = Module {
            type_section: vec![function_type(&[], &[ValueType::I32])],
            function_section: vec![0],
            table_section: vec![Table {
        ...
        assert_eq!(metadata.tables[0].ty, RefType::FUNCREF);
        assert_eq!(metadata.element_segments.len(), 1);
        assert_eq!(metadata.element_segments[0].table_index, 0);
        assert_eq!(metadata.element_segments[0].ty, RefType::FUNCREF);
        assert!(matches!(
            metadata.element_segments[0].mode,
            ElementMode::Active
        ));
        ...
        let matches = sidecar
            ...
            .collect::<Vec<_>>();
        assert_eq!(1, matches.len());
        let ref_type_offset = matches[0] + 4 + 4 + element.offset_expression.len() + 4;
        sidecar[ref_type_offset] = RefType::EXTERNREF.0;
    ...
    fn packaged_module_sidecar_rejects_corrupted_table_count_mismatch() {
        ...
        let metadata = compile_module_metadata(&module);
        assert_eq!(metadata.module_shape.local_table_count, 1);
        assert_eq!(1, metadata.tables.len());
        ...
        let matches = sidecar
            ...
            .collect::<Vec<_>>();
        assert_eq!(1, matches.len());
        sidecar[matches[0] + 32..matches[0] + 36].copy_from_slice(&2u32.to_le_bytes());
        ...
        let err = crate::aot::deserialize_aot_metadata(&decoded.modules[0].metadata_sidecar_bytes)
            .unwrap_err();
        assert_eq!(err.to_string(), "aot metadata: table count mismatch");
    }

    #[test]
    fn packaged_module_sidecar_rejects_corrupted_data_segment_count_mismatch() {
        let module = Module {
            type_section: vec![function_type(&[], &[ValueType::I32])],
            function_section: vec![0],
            memory_section: Some(razero_wasm::module::Memory {
        ...
        let metadata = compile_module_metadata(&module);
        assert_eq!(metadata.module_shape.data_segment_count, 1);
        assert_eq!(1, metadata.data_segments.len());
        ...
        let matches = sidecar
            ...
            .collect::<Vec<_>>();
        assert_eq!(1, matches.len());
        sidecar[matches[0] + 39..matches[0] + 43].copy_from_slice(&2u32.to_le_bytes());
        ...
        let err = crate::aot::deserialize_aot_metadata(&decoded.modules[0].metadata_sidecar_bytes)
            .unwrap_err();
        assert_eq!(err.to_string(), "aot metadata: data segment count mismatch");
    }
    ...
    fn packaged_module_sidecar_rejects_corrupted_any_memory_flag_mismatch() {
        ...
        let metadata = compile_module_metadata(&module);
        assert!(metadata.module_shape.has_any_memory);
        assert_eq!(1, metadata.data_segments.len());
    ...
    #[test]
    fn packaged_module_sidecar_rejects_corrupted_passive_element_with_offset_expression() {
        let module = Module {
            type_section: vec![function_type(&[], &[ValueType::I32])],
            function_section: vec![0],
            table_section: vec![Table {
        ...
        assert_eq!(metadata.element_segments.len(), 1);
        assert_eq!(
            metadata.element_segments[0].offset_expression,
            vec![0x41, 0x00, 0x0b]
        );
        assert!(matches!(
            metadata.element_segments[0].mode,
        ...
        let matches = sidecar
            ...
            .collect::<Vec<_>>();
        assert_eq!(1, matches.len());
        let mode_offset = matches[0] + 4 + 4 + element.offset_expression.len() + 4 + 1;
        sidecar[mode_offset] = 1;
    ...
    #[test]
    fn packaged_module_sidecar_rejects_corrupted_passive_element_with_nonzero_table_index() {
        let module = Module {
            type_section: vec![function_type(&[], &[ValueType::I32])],
            function_section: vec![0],
            table_section: vec![
        ...
        assert_eq!(
            err.to_string(),
            "aot metadata: passive/declarative element requires table_index == 0"
        );
    }

    #[test]
    fn packaged_module_sidecar_rejects_corrupted_element_with_empty_init_expressions() {
        let module = Module {
            type_section: vec![function_type(&[], &[ValueType::I32])],
            function_section: vec![0],
            table_section: vec![Table {
        ...
        let matches = sidecar
            ...
            .collect::<Vec<_>>();
        assert_eq!(1, matches.len());
        let init_count_offset = matches[0] + 4 + 4 + element.offset_expression.len() + 4 + 1 + 1;
        sidecar[init_count_offset..init_count_offset + 4].copy_from_slice(&0u32.to_le_bytes());
        let first_init_offset = init_count_offset + 4;
        sidecar.drain(first_init_offset..first_init_offset + 4 + element.init_expressions[0].len());
        ...
        assert_eq!(
            err.to_string(),
            "aot metadata: element init expressions cannot be empty"
        );
    }
    ...
    fn packaged_module_sidecar_rejects_corrupted_active_element_with_empty_offset() {
        ...
        let metadata = compile_module_metadata(&module);
        assert_eq!(metadata.element_segments.len(), 1);
        assert!(!metadata.element_segments[0].offset_expression.is_empty());
        assert!(matches!(
            metadata.element_segments[0].mode,
            ElementMode::Active
        ));
        ...
        let matches = sidecar
            ...
            .collect::<Vec<_>>();
        assert_eq!(1, matches.len());
        let offset_len_offset = matches[0] + 4;
        sidecar[offset_len_offset..offset_len_offset + 4].copy_from_slice(&0u32.to_le_bytes());
        sidecar
            .drain(offset_len_offset + 4..offset_len_offset + 4 + element.offset_expression.len());
        ...
        assert_eq!(
            err.to_string(),
            "aot metadata: active element requires offset expression"
        );
    }

    #[test]
    fn packaged_module_sidecar_rejects_corrupted_function_count_mismatch() {
        let module = Module {
            type_section: vec![function_type(&[], &[ValueType::I32])],
            function_section: vec![0],
            code_section: vec![Code {
        ...
        let metadata = compile_module_metadata(&module);
        assert_eq!(metadata.module_shape.local_function_count, 1);
        assert_eq!(1, metadata.functions.len());
        ...
        let matches = sidecar
            ...
            .collect::<Vec<_>>();
        assert_eq!(1, matches.len());
        sidecar[matches[0] + 24..matches[0] + 28].copy_from_slice(&2u32.to_le_bytes());
        ...
        let err = crate::aot::deserialize_aot_metadata(&decoded.modules[0].metadata_sidecar_bytes)
            .unwrap_err();
        assert_eq!(err.to_string(), "aot metadata: function count mismatch");
    }
    ...
    fn packaged_module_sidecar_rejects_corrupted_data_segment_passive_flag() {
        ...
        let matches = sidecar
            ...
            .collect::<Vec<_>>();
        assert_eq!(1, matches.len());
        let passive_flag_offset = matches[0] + 4;
        sidecar[passive_flag_offset] = 0xff;
    ...
    #[test]
    fn packaged_module_sidecar_rejects_corrupted_active_data_segment_with_empty_offset() {
        let module = Module {
            type_section: vec![function_type(&[], &[ValueType::I32])],
            function_section: vec![0],
            memory_section: Some(razero_wasm::module::Memory {
        ...
        sidecar.drain(
            offset_len_offset + 4
                ..offset_len_offset + 4 + metadata.data_segments[0].offset_expression.len(),
        );
        ...
        assert_eq!(
            err.to_string(),
            "aot metadata: active data segment requires offset expression"
        );
    }

    #[test]
    fn packaged_module_sidecar_rejects_corrupted_memory_shared_flag() {
        let module = Module {
            type_section: vec![function_type(&[], &[ValueType::I32])],
            function_section: vec![0],
            memory_section: Some(razero_wasm::module::Memory {
        ...
        let metadata = compile_module_metadata(&module);
        let memory = metadata.memory.as_ref().expect("memory metadata missing");
        assert!(memory.is_shared);

        let mut sidecar = serialize_aot_metadata(&metadata);
        let mut memory_pattern = Vec::new();
        memory_pattern.push(1);
        memory_pattern.extend_from_slice(&memory.min.to_le_bytes());
        memory_pattern.extend_from_slice(&memory.cap.to_le_bytes());
        memory_pattern.extend_from_slice(&memory.max.to_le_bytes());
        memory_pattern.push(1);
        memory_pattern.push(1);

        let matches = sidecar
            .windows(memory_pattern.len())
            .enumerate()
            .filter_map(|(offset, window)| (window == memory_pattern.as_slice()).then_some(offset))
            .collect::<Vec<_>>();
        assert_eq!(1, matches.len());
        let is_shared_offset = matches[0] + 1 + 4 + 4 + 4 + 1;
        sidecar[is_shared_offset] = 0xff;
        ...
        let err = crate::aot::deserialize_aot_metadata(&decoded.modules[0].metadata_sidecar_bytes)
            .unwrap_err();
        assert_eq!(err.to_string(), "aot metadata: invalid memory shared flag");
    }
    ...
    #[test]
    fn packaged_module_sidecar_rejects_invalid_start_function_index() {
        let module = Module {
            type_section: vec![function_type(&[], &[ValueType::I32])],
            function_section: vec![0; 8],
            code_section: vec![
        ...
        let matches = sidecar
            ...
            .collect::<Vec<_>>();
        assert_eq!(1, matches.len());
        sidecar[matches[0] + 1] = 8;
        sidecar[matches[0] + 2] = 0;
        sidecar[matches[0] + 3] = 0;
        sidecar[matches[0] + 4] = 0;
    ...
    #[test]
    fn packaged_module_sidecar_rejects_corrupted_helper_exit_code_flag() {
        let module = Module {
            type_section: vec![function_type(&[], &[ValueType::I32])],
            function_section: vec![0],
            code_section: vec![Code {
        ...
        let metadata = compile_module_metadata(&module);
        let helper = metadata.helpers[0].clone();
        assert!(helper.exit_code.is_some());

        let mut sidecar = serialize_aot_metadata(&metadata);
        let mut helper_pattern = Vec::new();
        helper_pattern.push(1);
        helper_pattern.extend_from_slice(&helper.execution_context_offset.to_le_bytes());
        helper_pattern.push(1);
        helper_pattern.extend_from_slice(
            &helper
                .exit_code
        ...
        let matches = sidecar
            .windows(helper_pattern.len())
            .enumerate()
            .filter_map(|(offset, window)| (window == helper_pattern.as_slice()).then_some(offset))
            .collect::<Vec<_>>();
        assert_eq!(1, matches.len());
        let exit_code_flag_offset = matches[0] + 1 + 4;
        sidecar[exit_code_flag_offset] = 0xff;
        ...
        assert_eq!(
            err.to_string(),
            "aot metadata: invalid helper exit-code flag"
        );
    }
    ...
    #[test]
    fn link_native_executable_rejects_mismatched_global_initializer_type() {
        let module = Module {
            type_section: vec![function_type(&[ValueType::I32], &[ValueType::I32])],
            function_section: vec![0],
            memory_section: Some(razero_wasm::module::Memory {
        ...
        let err = link_native_executable(
            PathBuf::from("target/mismatched-global-init-native-bin"),
            &[NativeLinkModule::new(
                "guest",
                Vec::new(),
                serialize_aot_metadata(&metadata),
        ...
        assert_eq!(
            err.to_string(),
            "module 'guest' is outside the packaged linked-runtime slice: global[0] initializer type i64 does not match declared type i32"
        );
    }

    #[test]
    fn link_native_executable_rejects_invalid_data_offset_opcode() {
        let module = Module {
            type_section: vec![function_type(&[ValueType::I32], &[ValueType::I32])],
            function_section: vec![0],
            memory_section: Some(razero_wasm::module::Memory {
        ...
        let err = link_native_executable(
            PathBuf::from("target/invalid-data-offset-opcode-native-bin"),
            &[NativeLinkModule::new(
                "guest",
                Vec::new(),
                serialize_aot_metadata(&metadata),
        ...
        assert_eq!(
            err.to_string(),
            "module 'guest' is outside the packaged linked-runtime slice: invalid opcode for const expression: 0xff"
        );
    }
    ...
    #[test]
    fn link_native_executable_rejects_module_with_non_scalar_result_export_signature() {
        let module = Module {
            type_section: vec![function_type(&[ValueType::I32], &[ValueType::I32])],
            function_section: vec![0],
            code_section: vec![Code {
        ...
        let err = link_native_executable(
            PathBuf::from("target/non-scalar-result-native-bin"),
            &[NativeLinkModule::new(
                "guest",
                Vec::new(),
                serialize_aot_metadata(&metadata),
