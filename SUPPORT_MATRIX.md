...
- Public guest-import calls currently do **not** emit a separate imported-host
  `FunctionListener.Before` / `After` callback. The caller's listener still
  wraps the overall guest export call, and a denied host import therefore
  reports the policy decision and aborts only that caller listener.
...
- `YieldPolicyObserver`, when present, reports the cooperative-yield allow/deny
  ...
  and before the corresponding `YieldObserver::Yielded` callback when the yield
  is allowed.
...
- Install hardening hooks at the narrowest scope that matches their job:
  ...
  | instantiation context | `experimental::with_import_resolver_acl`, `experimental::with_import_resolver_config`, `experimental::with_import_resolver_observer` | Import resolution happens only while instantiating a module, so attach ACL/resolver policy and observation to `compile`, `instantiate`, or `instantiate_with_context` flows. |
  | call / resume context | `experimental::with_yielder`, `experimental::with_trap_observer`, `experimental::with_yield_observer`, `experimental::with_fuel_controller`, `experimental::with_fuel_observer`, `experimental::with_host_call_policy`, `experimental::with_host_call_policy_observer`, `experimental::with_yield_policy`, `experimental::with_yield_policy_observer`, `experimental::with_time_provider` | These govern a specific execution attempt and can change on `Resume`. Rebuild the resume context with the observers/policies you still want active. |
