...
func TestModule_Global(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := r.store.Engine.CompileModule(testCtx, code.module, nil, false, 0, false)
			require.NoError(t, err)
...
func (e *mockEngine) CompileModule(_ context.Context, module *wasm.Module, _ []experimental.FunctionListener, _ bool, _ int64, _ bool) error {
