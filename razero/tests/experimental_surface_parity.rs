...
#[test]
fn import_resolver_observer_emits_acl_denied_through_public_surface() {
    let observed = Arc::new(AtomicU32::new(0));
    let ctx = with_import_resolver_observer(&Context::default(), {
        let observed = observed.clone();
        move |_ctx: &Context, observation: ImportResolverObservation| {
            assert_eq!(ImportResolverEvent::AclDenied, observation.event);
            observed.fetch_add(1, Ordering::SeqCst);
        }
    });
    let observer = get_import_resolver_observer(&ctx).expect("observer should be present");
...
#[test]
fn import_resolver_observer_emits_acl_allowed_through_public_surface() {
    let observed = Arc::new(AtomicU32::new(0));
    let ctx = with_import_resolver_observer(&Context::default(), {
        let observed = observed.clone();
        move |_ctx: &Context, observation: ImportResolverObservation| {
            assert_eq!(ImportResolverEvent::AclAllowed, observation.event);
            observed.fetch_add(1, Ordering::SeqCst);
        }
    });
    let observer = get_import_resolver_observer(&ctx).expect("observer should be present");
...
#[test]
fn import_resolver_observer_emits_resolver_attempted_through_public_surface() {
    let observed = Arc::new(AtomicU32::new(0));
    let ctx = with_import_resolver_observer(&Context::default(), {
        let observed = observed.clone();
        move |_ctx: &Context, observation: ImportResolverObservation| {
            assert_eq!(ImportResolverEvent::ResolverAttempted, observation.event);
...
#[test]
fn import_resolver_observer_emits_resolver_resolved_through_public_surface() {
    let observed = Arc::new(AtomicU32::new(0));
    let runtime = Runtime::new();
    let compiled = runtime.compile(SIMPLE_EXPORT_WASM).unwrap();
    let resolved_module = runtime
        .instantiate(&compiled, ModuleConfig::new().with_name("env"))
        .unwrap();
    let ctx = with_import_resolver_observer(&Context::default(), {
        let observed = observed.clone();
        move |_ctx: &Context, observation: ImportResolverObservation| {
            assert_eq!(ImportResolverEvent::ResolverResolved, observation.event);
            assert!(observation.resolved_module.is_some());
            observed.fetch_add(1, Ordering::SeqCst);
        }
    });
    let observer = get_import_resolver_observer(&ctx).expect("observer should be present");
