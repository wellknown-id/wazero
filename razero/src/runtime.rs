...
mod tests {
    ...
    #[test]
    fn allowed_guest_host_callbacks_do_not_emit_separate_import_listener_callbacks() {
        let runtime = Runtime::new();
        ...
        let policy_ctx = with_host_call_policy(&listener_ctx, allow_all_host_calls);
        let ctx = with_host_call_policy_observer(&policy_ctx, {
            let observations = observations.clone();
            move |_ctx: &Context, observation: HostCallPolicyObservation| {
                observations
        ...
        let listener_events = listener_events.lock().expect("listener events poisoned");
        assert_eq!(2, listener_events.len());
        assert!(listener_events[0].starts_with("before:run:[41]:"));
        assert_eq!("after:run:[42]", listener_events[1]);
        assert!(listener_events.iter().all(|event| !event.contains(":inc:")));
    }

    #[test]
    fn denied_guest_host_callbacks_abort_only_caller_listener() {
        let runtime = Runtime::new();
        let listener_events = Arc::new(Mutex::new(Vec::new()));
        runtime
            .new_host_module_builder("env")
        ...
        let ctx = with_host_call_policy(&listener_ctx, deny_env_host_calls);
        ...
        assert_eq!(2, listener_events.len());
        assert!(listener_events[0].starts_with("before:run:[41]:"));
        assert_eq!("abort:run:policy denied: host call", listener_events[1]);
        assert!(listener_events.iter().all(|event| !event.contains(":inc:")));
    }
    ...
    #[test]
    fn host_call_policy_observer_resume_context_receives_follow_on_denial() {
        let runtime = Runtime::new();
    ...
    #[test]
    fn yielded_guest_call_while_suspended_is_rejected() {
        let runtime = Runtime::new();
        runtime
            .new_host_module_builder("example")
            .new_function_builder()
    ...
    #[test]
    fn yielded_guest_other_export_while_suspended_is_rejected() {
        let runtime = Runtime::new();
        runtime
            .new_host_module_builder("example")
            .new_function_builder()
    ...
    fn trap_observer_resume_context_receives_follow_on_policy_denial() {
        ...
        let initial_ctx = with_trap_observer(&with_yielder(&Context::default()), {
            let initial_observations = initial_observations.clone();
            move |_ctx: &Context, observation: TrapObservation| {
                initial_observations
                    .lock()
    ...
    fn yield_policy_observer_resume_context_receives_follow_on_denial() {
        ...
        let initial_ctx = with_yield_policy_observer(&with_yielder(&Context::default()), {
            let initial_observations = initial_observations.clone();
            move |_ctx: &Context, observation: YieldPolicyObservation| {
                initial_observations
                    .lock()
